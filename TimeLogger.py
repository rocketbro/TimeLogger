...
class Client:
	...
	# init method
	def __init__(self, tag, remaining_hours):
		self.tag = str(tag)
		if remaining_hours.isdigit():
			self.remaining_hours = float(remaining_hours)
	
	# display tag, useful for printing in the console
	# and the log text file
	@property
	def tag_display(self):
		return f"Client: {self.tag}\n"
		
	# path to this client's log file
	@property
	def log_file_path(self):
		return f"tl_log_{self.tag}.txt"
		
	# update remaining hours
	def update_remaining_hours(self, new_value):
		self.remaining_hours = float(new_value)
		
	# update total hours tracked for client
	def update_total_hours_tracked(self, new_value):
		self.total_hours_tracked = float(new_value)
		
		
		
tl_data = {
			"current_client_id" : "N/A",
			"current_client_id_display" : "",
	...
	def save(self):
		try:
			with open(self.data_file_path, "w") as data_file:
				json.dump(self, data_file, default = lambda o: o.__dict__, indent = 4)
...
def loadLocalData():
	...
	# it should be safely called in a try except block.
...
def recover():
	...
	execute_recovery = confirmationDialog("Do you want to use the last saved start time as your current task start time?", "Please enter")
	if execute_recovery:
		if start_time is tl_data['saved_start_time']:
...
def createClient():
	...
	new_client_id = input("\nEnter a client ID: ")
	while new_client_id is "":
		new_client_id = input("Client ID must contain at least one character.\nEnter a client ID: ")
		
	client_hours_remaining = input("Enter a total number of client hours to count down from, or press RETURN to skip: ")
	client_hours_remaining = checkForFloatOrSkip(client_hours_remaining)
	
	new_client = {
		"id" : new_client_id,
		"id_display" : f"Client: {new_client_id}\n",
		"log_file_path" : f"tl_log_{new_client_id}.txt"
	}
	
	tl_data['clients'][new_client_id] = new_client
	save()
	
	with open(tl_data['clients'][new_client_id]['log_file_path'], "w") as log_file:
		client = tl_data['clients'][new_client_id]
		log_file.seek(0)
		log_file.write(f"TIME LOGGER VER 2.0.1\nTime Log\n")
		if tl_data['username'] is not "":
			log_file.write(f"\n{tl_data['username']}")
		if tl_data['email'] is not "":
			log_file.write(f"\n{tl_data['email']}\n")
		log_file.write(client['id_display'])
		
	tl_data['log_file_path'] = tl_data['clients'][new_client_id]['log_file_path']
	tl_data['current_client_id'] = tl_data['clients'][new_client_id]['id']
	tl_data['current_client_id_display'] = tl_data['clients'][new_client_id]['id_display']
	save()
			
	print(f"Saved. Current ")

# define initializtion method to configure Time Logger's initial state
def initialize(bypassConfirmation = False):
	if bypassConfirmation:
		confirmed = True
	else:
		confirmed = confirmationDialog("WARNING: THIS MAY OVERWRITE ANY EXISTING LOCAL TIME LOGGER FILES.")
	...
	if confirmed:
		...
		print("You may press RETURN to skip any of the following fields. Any information entered will exist only in files local to the current directory.\n")
		...
		# ask for a client id
		client_id = input("Add a client ID if you want to track time for a specific client.\nClient ID: ")
		
		# if a client id is specified, update the data dictionary
		if client_id != "":
			tl_data["current_client_id"] = client_id
			tl_data["current_client_id_display"] = f"Client: {client_id}\n"
			tl_data["log_file_path"] = f"tl_log_{client_id}.txt"
		
		# ask for initial hours
		initial_hours = input("Optionally enter a total number of hours to count down from: ")
		initial_hours = checkForFloatOrSkip(initial_hours)
		tl_data["remaining_hours"] = initial_hours
		if initial_hours is not "":
		...
		with open(tl_data["log_file_path"], "w") as log_file:
			client = tl_data["current_client_id_display"]
			log_file.seek(0)
			log_file.write(f"TIME LOGGER VER 2.0.1\nTime Log\n")
			if tl_data['username'] is not "":
				log_file.write(f"\n{tl_data['username']}")
...
commands = {
	...
	"big red button": initialize,
	"recover" : recover,
	"tog rem" : toggleTotalHourDisplay,
	"tog tot" : toggleRemainingHourDisplay,
...
def checkForFloatOrSkip(value):
	if value == "":
		return 0
	elif value.isdigit():
		return float(value)
	else:
		checkForFloatOrSkip(input("\nPlease enter a valid number or hit RETURN to skip: "))
...
def startNewTask(isRecoveryTask = False):
	...
	if tl_data["unterminated_task"]:
		use_saved_start = confirmationDialog("An unterminated task has been detected. Do you want to use the last saved start time as your current task start time?", "Please enter")
...
{tl_data["current_client_id_display"]}
...
""")	
	...
	while value != "Q":
		...
		tl_data['remaining_hours'] = max((tl_data['remaining_hours'] - ensureHourFormat(total_time, format)), 0)
...
try:
	...
	print(f"{tl_splash}\nWelcome to Time Logger! A few quick questions to get you rolling...\n")
	bypassConfirmation = True
	initialize(bypassConfirmation)
	
except Exception as error:
	print(f"A serious error has occured:\n\n{error}\n\nTime Logger will reinitialize.\n")
	initialize()
